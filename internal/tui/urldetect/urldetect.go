...
// pattern matches bare http(s) URLs, stopping at whitespace or ESC (so ANSI
// escapes around styled URLs are never swallowed into the match).
var pattern = regexp.MustCompile(`https?://[^\s` + string(rune(0x1b)) + `]+`)
